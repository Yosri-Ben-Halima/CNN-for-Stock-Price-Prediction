...
def plot_percentage_error(date, pe, fig, row=3, col=1):
    """
    Plots the percentage error (PE) between actual and predicted values on a specified subplot of a figure.

    Parameters
    ----------
    date : array-like
        An array or list of dates corresponding to the PE values.
    pe : array-like
        An array or list of percentage errors (PE) values.
    fig : plotly.graph_objects.Figure
    ...
    Description
    -----------
    This function adds a trace to a Plotly figure to visualize the percentage error (PE) over time.
    It creates a line plot showing the percentage difference between the actual and predicted values.
    The x-axis is labeled as 'Time', and the y-axis is labeled as 'PE (%)'.
    The trace is added to the subplot specified by the `row` and `col` parameters.
    """

    fig.add_trace(
        go.Scatter(x=date, y=pe, mode='lines', name='PE (%)'),
        row=row, col=col
    )

    fig.update_xaxes(title_text='Time', row=row, col=col)
    fig.update_yaxes(title_text='PE (%)', row=row, col=col)
...
def plot_monitoring_dashboard(history, predictions_df, ticker):
    ...
    predictions_df : pandas.DataFrame
        DataFrame containing columns 'Actual', 'Predicted', and 'PE' (Percentage Error), 
    ...
    - The third subplot illustrates the Percentage Error (PE) between the actual and predicted prices.
    ...
    fig = make_subplots(
        ...
        subplot_titles=("Prediction vs. Actual Stock Price", "Prediction vs. Actual Stock Price Movement", "Percentage Error", "Model Loss"),
    ...
    plot_percentage_error(predictions_df.index, predictions_df['PE'] ,fig)
