...
def percentage_error(predictions_df):
    ...
    None
        ...
        - 'MPE': The Mean Percentage Error for each prediction, calculated as
          100 * (Actual - Predicted) / Actual.

    Description
    -----------
    This function computes the Mean Percentage Error (MPE) for each entry in the
    DataFrame. The MPE is calculated by comparing the actual values with the
    predicted values and is expressed as a percentage. The result is stored in
    a new column 'MPE' within the input DataFrame.

    Note
    ----
    The input DataFrame is modified in place, and the function does not return
    any value.
    """

    predictions_df['MPE'] = 100*(predictions_df['Actual']-predictions_df['Predicted'])/predictions_df['Actual']
